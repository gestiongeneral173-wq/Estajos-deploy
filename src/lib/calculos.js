...
export function rosterToList(roster) {
  ...
  if (roster.encargado) {
    ...
    filas.push({ id: roster.encargado, roles, ...roster.horasRoles?.[roster.encargado] });
    vistos.add(roster.encargado);
  }

  if (roster.chofer && !vistos.has(roster.chofer)) {
    filas.push({ id: roster.chofer, roles: ["C"], ...roster.horasRoles?.[roster.chofer] });
    vistos.add(roster.chofer);
  }

  for (const pasajero of roster.pasajeros || []) {
    filas.push({ id: pasajero.id, roles: [], horas: pasajero.horas, destajo: pasajero.destajo });
  }
...
const hastaElCierre = (cierre) => (fecha) => !cierre || !fecha || fecha <= cierre;
...
export function calcEmpleado(state, empleadoId) {
  ...
  const dentro = hastaElCierre(cierreDeCiclo(state, trabajador?.payment_period ?? "quincenal"));
  const jornadas = state.jornadas.filter(
    (j) => j.empleado_id === empleadoId && !j.fue_liquidado && dentro(j.fecha),
  );
  const adelantos = state.adelantos.filter((a) => a.empleado_id === empleadoId && dentro(a.fecha));
  const totalDevengado = jornadas.reduce(
    (total, j) => total + j.horas * j.tarifa + Number(j.destajo),
    0,
  );
  const totalAdelantos = adelantos.reduce((total, a) => total + Number(a.monto), 0);
...
export function calcVehiculo(state, vehiculoId) {
  ...
  const dias = state.plazasVehiculo.filter((d) => d.vehiculo_id === vehiculoId && dentro(d.fecha));
  const adelantos = state.adelantosVehiculo.filter(
    (a) => a.vehiculo_id === vehiculoId && dentro(a.fecha),
  );
  const totalDevengado = dias.reduce((total, d) => total + d.plazas * d.tarifa_aplicada, 0);
  const totalAdelantos = adelantos.reduce((total, a) => total + Number(a.monto), 0);
