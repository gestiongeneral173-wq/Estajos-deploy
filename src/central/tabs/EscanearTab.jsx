...
export function AdelantosView({ trabajadorId, adelantos, actions, onBack, pendiente = 0 }) {
  const [monto, setMonto] = useState(""),
    ...
    registrar = () => {
      ...
      actions.registrarAdelanto(trabajadorId, importe);
      setMonto("");
      onBack();
    };
  return (
    <Card>
      ...
      <div className="space-y-3">
        ...
        <p className="text-[11px] text-gray-500 text-center">
          El adelanto se registra automáticamente con la fecha de hoy.
        </p>
        <div className="grid grid-cols-2 gap-3">
          <Button variant="outline" icon={<ArrowLeft className="w-4 h-4" />} onClick={onBack}>
            Volver
          </Button>
          <Button variant="primary" disabled={!monto} onClick={registrar}>
            Registrar
          </Button>
        </div>
      </div>
